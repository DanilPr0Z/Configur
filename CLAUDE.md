...
`frontend/src/components/scheme.test.ts` — отрисовка плана и развёртки:
рендер в строку через `renderToStaticMarkup` (jsdom не нужен), проверка, что в
SVG нет NaN/Infinity и что битые данные старого заказа не ломают чертёж.
